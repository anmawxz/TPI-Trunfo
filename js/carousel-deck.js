...
arrowIcons2.forEach(icon => {
    icon.addEventListener("click", () => {
        let firstImg2Width = firstImg2.clientWidth + 14; // getting first img width & adding 14 margin value
...
const showHideIcons3 = () => {
    // showing and hiding prev/next icon according to carousel3 scroll left value
...
arrowIcons3.forEach(icon => {
    icon.addEventListener("click", () => {
        let firstImg3Width = firstImg3.clientWidth + 14; // getting first img width & adding 14 margin value
        // if clicked icon is left, reduce width value from the carousel3 scroll left else add to it
        carousel3.scrollLeft += icon.id == "left" ? -firstImg3Width : firstImg3Width;
...
const autoSlide3 = () => {
    ...
    let firstImg3Width = firstImg3.clientWidth + 14;
    // getting difference value that needs to add or reduce from carousel3 left to take middle img center
    let valDifference = firstImg3Width - positionDiff3;

    if(carousel3.scrollLeft > prevScrollLeft3) { // if user is scrolling to the right
        return carousel3.scrollLeft += positionDiff3 > firstImg3Width / 3 ? valDifference : -positionDiff3;
    }
    // if user is scrolling to the left
    carousel3.scrollLeft -= positionDiff3 > firstImg3Width / 3 ? valDifference : -positionDiff3;
}
...
firstImg4 = carousel4.querySelectorAll("img")[0],
...
const showHideIcons4 = () => {
    // showing and hiding prev/next icon according to carousel4 scroll left value
    let scrollWidth = carousel4.scrollWidth - carousel4.clientWidth; // getting max scrollable width
    arrowIcons4[0].style.display = carousel4.scrollLeft == 0 ? "none" : "block";
    arrowIcons4[1].style.display = carousel4.scrollLeft == scrollWidth ? "none" : "block";
}

arrowIcons4.forEach(icon => {
    icon.addEventListener("click", () => {
        let firstImg4Width = firstImg4.clientWidth + 14; // getting first img width & adding 14 margin value
        // if clicked icon is left, reduce width value from the carousel4 scroll left else add to it
        carousel4.scrollLeft += icon.id == "left" ? -firstImg4Width : firstImg4Width;
        setTimeout(() => showHideIcons4(), 60); // calling showHideIcons4 after 60ms
    });
});
...
const autoSlide4 = () => {
    ...
    positionDiff4 = Math.abs(positionDiff4); // making positionDiff4 value to positive
    let firstImg4Width = firstImg4.clientWidth + 14;
    // getting difference value that needs to add or reduce from carousel4 left to take middle img center
    let valDifference = firstImg4Width - positionDiff4;

    if(carousel4.scrollLeft > prevScrollLeft4) { // if user is scrolling to the right
        return carousel4.scrollLeft += positionDiff4 > firstImg4Width / 3 ? valDifference : -positionDiff3;
    }
    // if user is scrolling to the left
    carousel4.scrollLeft -= positionDiff4 > firstImg4Width / 3 ? valDifference : -positionDiff4;
}
...
const dragging4 = (e) => {
    // scrolling images/carousel4 to left according to mouse pointer
